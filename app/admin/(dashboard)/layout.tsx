...
export default function AdminDashboardLayout({
  ...
  const navItems = [
    ...
    { name: 'Powerteams', href: '/admin/classes', icon: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z' },
  ...
  const [appName, setAppName] = React.useState('Atoms Admin')
    ...
    fetch('/api/settings')
      ...
      .then(data => {
        if (data.success && data.settings) {
          setAppName(`${data.settings.appName} Admin`)
        }
      })
    ...
    <div className="flex h-screen bg-gray-50 overflow-hidden font-sans">
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-50 flex md:hidden">
          <div className="fixed inset-0 bg-gray-800 bg-opacity-50 transition-opacity" onClick={() => setIsMobileMenuOpen(false)}></div>
          <aside className="relative flex-1 flex flex-col max-w-xs w-full bg-white shadow-xl transform transition-transform">
            <div className="h-16 flex items-center justify-between px-6 border-b border-gray-100 shrink-0">
              <span className="text-xl font-extrabold text-gray-900 tracking-tight">{appName}</span>
              <button onClick={() => setIsMobileMenuOpen(false)} className="text-gray-500 hover:text-gray-900 p-2 -mr-2">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Mobile Header */}
        <header className="md:hidden h-16 flex items-center justify-between bg-white border-b border-gray-200 px-4 shrink-0 shadow-sm z-30">
          <span className="text-lg font-extrabold text-gray-900 tracking-tight">{appName}</span>
          <button onClick={() => setIsMobileMenuOpen(true)} className="text-gray-600 hover:text-gray-900 focus:outline-none p-2 rounded-md hover:bg-gray-50">
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
