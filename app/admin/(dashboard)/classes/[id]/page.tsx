...
export default function ClassDetailsPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const fetchData = async () => {
        try {
            const [classRes, membersRes] = await Promise.all([
                fetch(`/api/class/${id}`),
                fetch('/api/member') // Assuming this endpoint returns all members, or we can use another one
            ]);
            
            const classJson = await classRes.json();
            if (classJson.success) setClassData(classJson.class);
    ...
    if (loading) return <div className="p-8">Loading powerteam details...</div>;
    if (!classData) return <div className="p-8">Powerteam not found</div>;
                            ...
                            {classData.members.length === 0 ? (
                                ...
                                classData.members.map((member: any) => (
                                    <div key={member.memberId} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-100">
                                        ...
                                        <button onClick={() => handleRemove(member.memberId)} className="text-xs font-bold text-red-600 hover:text-red-800 px-2 py-1">Remove</button>
                                    </div>
                                ))
                            )}
                        </div>
                                ...
                                {searchResults.length === 0 ? (
                                    ...
                                    searchResults.map((member: any) => (
                                        <div key={member.memberId} className="flex items-center justify-between p-3 bg-blue-50 rounded-lg border border-blue-100">
                                            ...
                                            <button onClick={() => handleAssign(member.memberId)} className="bg-blue-600 text-white text-xs font-bold px-3 py-1.5 rounded hover:bg-blue-700 transition">Add</button>
                                        </div>
                                    ))
                                )}
                            </div>
